...
- **บั๊กที่เจอระหว่างทดสอบจริง (ไม่เกี่ยวกับ Library เอง แต่โผล่มาให้เห็นเพราะเป็นทางแรกที่ exercise เคสเหล่านี้จริงจัง):**
  ...
  3. **Legacy `OBJECT_TYPE_PES2_TEXT` (type=0, "Font Stitch" จากยุค PES2/Pinn) โผล่ใน AutoSewing templates เป็นช่องใส่ชื่อลูกค้าที่ยังไม่ digitize** (ฟอนต์/สี/ตำแหน่งครบ แต่ 0 ฝีเข็ม, bbox จิ๋ว 0.2mm). พอร์ตขาด migration ที่แอปเก่าทำ (`PES5_ReplaceWithDefaultFont`, เรียกทันทีหลัง `ImportDataAs` ใน `DlgPPESHandler.js`). แก้ด้วย `pescore::migratePes2TextObjects()` ใน `pes_text_core.hpp` — แปลงเป็น `SCALABLE_PPEF_TEXT` (ฟอนต์ Thai001, satin fill) ตอนโหลด `.ppes`, เรียกจาก `load_ppes` (native) + `web_load_input` (web). Web ต้อง fetch Thai001.ppef on-demand → เพิ่ม `pes2TextMigrationNeedsFont()` คืน `missing_font` sentinel + retry loop ใน `webLoadInput` (webEngine.ts). **แก้ latent bug ของแอปเก่าไปด้วย**: อ่าน `centerx`/`centery` ดิบก่อน `rebuildPpefText()` clobber ค่า (ผ่าน `getBoundingBox()`) → reposition กลับตำแหน่ง template จริง (ไม่งั้นเด้งไปกลางสะดึง)
  4. **`__showJumpStitch` default `true` → `false`** (`pesDocument.cpp`): per-object snapshot เดิมโชว์เส้น jump/trim ทแยงเชื่อมชิ้นงานที่ไม่ได้ปักจริง; thumbnail renderer (`getThumbnailPNGBuffer`) hardcode hide อยู่แล้ว — ให้ canvas + layer thumbnail สะอาดตรงกัน
